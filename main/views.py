...
def show_main(request):
    flower_entries = Shop.objects.filter(user=request.user)

    context = {
        'name': request.user.username,
        'class': 'PBP A',
        'npm': '2306245642',
        'flower_entries': flower_entries,
...
def create_flower(request):
    ...
    if form.is_valid() and request.method == "POST":
        flower_entry = form.save(commit=False)
        flower_entry.user = request.user
        flower_entry.save()
        return redirect('main:show_main')

    context = {'form': form}
    return render(request, "create_flower.html", context)
